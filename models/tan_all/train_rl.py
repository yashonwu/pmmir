...
from models.tran_all.model_transformer import RecTransformer
from models.tran_all.eval_utils import evaluation
from models.tran_all.train_utils import train_sl
...
def train_rl(epoch, optimizer):
    ...
    for batch_idx in range(1, num_epoch + 1):
        ...
        # user.sample_idx(user_img_idx, split)
        history_input,_ = user.sample_target_idx_history(user_img_idx, split, batch_idx, args.batch_size, total_batch, history_idx)
                
        if torch.cuda.is_available():
            history_input = history_input.cuda()
        init_state = model.init_forward(history_input)
        # sampling for the next turn
        top_k_act_img_idx = ranker.k_nearest_neighbors(init_state.data,K=args.top_k)
        ...
        for k in range(dialog_turns):
            ...
            if k==0:
                ...
                # log_prob, reward
                log_prob_init, reward_init = dist(init_state, user_img_idx, top_k_act_img_idx, implicit_memory_act_img_idx)

                prob_list.append(log_prob_init)
                reward_list.append(reward_init)
            # else:
            ...
            # prob_init_list.append(log_prob_init)
        ...
        for i, prob_temp in enumerate(reversed_prob_list):
            cum_reward = reward_list[dialog_turns+1-1-i] + args.gamma * cum_reward
            policy_loss = prob_temp * cum_reward
            loss_sum = policy_loss + loss_sum
            # + prob_init_list[dialog_turns-1-i]
