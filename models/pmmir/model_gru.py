...
class RecGRU(nn.Module):
    def __init__(self, top_k):
        ...
        self.hist_rnn = nn.GRUCell(self.dim_size, self.dim_size, bias=False)
        self.hist_head = nn.Linear(in_features=self.dim_size, out_features=self.dim_size, bias=False)
...
#         return image_emb
    ...
    def init_forward(self, history_input):
        ...
        for i in range(history_input.size(1)):
            x = image_emb[:,i,:]
            self.hx = self.hist_rnn(x, self.hx)
            
        outs = self.hist_head(self.hx)
        return outs
