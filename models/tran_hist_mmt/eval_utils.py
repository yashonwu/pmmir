...
def evaluation(split, model_mmt, model_tran, user, ranker, batch_size, top_k, turns, model_dir, result_folder):
    model_mmt.eval()
    model_tran.eval()
    ...
    for batch_idx in range(1, total_batch + 1):
        ...
        feat_tran = model_tran.update_rep(all_input)
        ranker.update_rep(feat_tran)
        ...
        state = model_tran.init_forward(history_input)
        # sampling for the next turn
        top_k_act_img_idx = ranker.k_nearest_neighbors(state.data,K=top_k)
        
        # update item embeddings
        feat_mmt = model_mmt.update_rep(all_input)
        ranker.update_rep(feat_mmt)
        ...
        for k in range(dialog_turns):
            ...
            with torch.no_grad():
                state = model_mmt.merge_forward(act_input, txt_input)
